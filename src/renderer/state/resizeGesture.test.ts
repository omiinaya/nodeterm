...
describe('resizeGesture', () => {
  it('begins a gesture with a flat prevRect (first frame == current)', () => {
    useResizeGesture.getState().begin('node-a', RECT)
    expect(useResizeGesture.getState().gesture).toEqual({ nodeId: 'node-a', rect: RECT, prevRect: RECT })
  })

  it('advances rect and carries the previous frame as prevRect', () => {
